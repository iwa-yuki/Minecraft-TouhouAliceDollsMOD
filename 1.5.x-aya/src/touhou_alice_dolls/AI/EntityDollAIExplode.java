...
public class EntityDollAIExplode extends EntityDollAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        if(theDoll.getDollID() != DollRegistry.getDollID("Ooedo"))
        {
            return false;
        }
        ItemStack subItem = theDoll.getSubItem();
        if(subItem != null && subItem.itemID == Item.cake.itemID)
        {
            return false;
        }
    ...
    public void updateTask()
    {
        ...
        if(this.theDoll.getDistanceSqToEntity(this.theTarget) < 5f)
        {
            if(fuse == 1)
            {
                theDoll.playSE("random.fuse", 1.0F, 0.5F);
            }
            if(fuse >= 30)
            {
                if (!theWorld.isRemote)
                {
                    ...
                    ItemStack subItem = theDoll.getSubItem();
                    if(subItem != null && subItem.itemID == Block.tnt.blockID)
                    {
                    ...
                    theDoll.onDeath(DamageSource.func_94539_a(exp));
                    theDoll.setDead();
                }
            }
            ++fuse;
