...
public class EntityDollAISearchSpawner extends EntityDollAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        if(theDoll.getDollID() != DollRegistry.getDollID("London"))
        {
            return false;
        }
        return true;
        ...
        for(int dx = -16; dx <= 32; ++dx)
        {
            for(int dz = -16; dz <= 32; ++dz)
            {
                for(int dy = -1; dy <= 1; dy+=2)
                {
                    ...
                    int bid = theWorld.getBlockId(chunkX*16+dx, baseY+counter*dy, chunkZ*16+dz);
                    if(bid == Block.mobSpawner.blockID)
                    {
                        TileEntityMobSpawner spawner = (TileEntityMobSpawner)(theWorld.getBlockTileEntity(chunkX*16+dx, baseY+counter*dy, chunkZ*16+dz));
                        MobSpawnerBaseLogic spawnerLogic = spawner.func_98049_a();
                        
                        if(spawner != null)
                        {
                            String mobname = spawnerLogic.func_98276_e();
                            if(mobname == null)
                            {
                                mobname = "unknown";
                            }
	...
	public String getStrongholdDirection()
	{
		...
		double vecX = chunkposition.x - theDoll.posX;
		double vecY = chunkposition.y - theDoll.posY;
		double vecZ = chunkposition.z - theDoll.posZ;
